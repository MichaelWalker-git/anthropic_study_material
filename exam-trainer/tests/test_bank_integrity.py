"""
Цілісність банку питань (questions.json).

Це найфундаментальніший рівень: якщо дані биті, жодна фіча вище не має сенсу.
Перевіряємо ІНВАРІАНТИ, які мусять виконуватись для КОЖНОГО з ~457 питань.
"""
...
def test_required_fields_present(app_module, field):
    ...
    assert not missing, f"поле '{field}' відсутнє в питаннях: {missing[:10]}"


def test_exactly_four_distinct_options(app_module):
    for q in app_module.QUESTIONS:
        opts = q["options"]
        assert set(opts) == {"A", "B", "C", "D"}, f"q{q['id']}: варіанти != A-D"
        texts = [t.strip().lower() for t in opts.values()]
        assert len(set(texts)) == 4, f"q{q['id']}: варіанти не унікальні"


def test_correct_letter_is_valid(app_module):
    for q in app_module.QUESTIONS:
        assert q["correct"] in {"A", "B", "C", "D"}, f"q{q['id']}: погана правильна літера"
        assert q["correct"] in q["options"], f"q{q['id']}: правильна літера поза options"


def test_why_is_nonempty(app_module):
    for q in app_module.QUESTIONS:
        assert q["why"] is not None and q["why"].strip(), f"q{q['id']}: порожнє пояснення"


def test_mock_exam_questions_have_per_option_explanations(app_module):
    """Питання з mock-exam мають explanations до кожного варіанту (для chosen_why)."""
    for q in app_module.QUESTIONS:
        if q.get("source") != "mock-exam":
            continue
        expl = q.get("explanations")
        assert expl and set(expl) == {"A", "B", "C", "D"}, \
            f"q{q['id']}: mock-exam має пояснення до всіх 4 варіантів"


def test_scenarios_are_canonical(app_module):
    """Назви сценаріїв не мають дублюватись через регістр/варіанти написання."""
    scenarios = {q["scenario"] for q in app_module.QUESTIONS}
    lowered = {s.lower() for s in scenarios}
    assert len(scenarios) == len(lowered), \
        f"є сценарії, що відрізняються лише регістром: {sorted(scenarios)}"


def test_answer_not_trivially_leaked_in_prompt(app_module):
    """Текст правильної відповіді не має дослівно бути в питанні (грубий витік)."""
    leaks = []
    for q in app_module.QUESTIONS:
        correct_text = q["options"][q["correct"]].strip().lower()
        if correct_text and len(correct_text) > 25 and correct_text in q["prompt"].lower():
            leaks.append(q["id"])
    assert not leaks, f"правильна відповідь дослівно в питанні: {leaks}"
